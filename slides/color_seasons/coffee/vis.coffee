...
DiffPlot = () ->
  ...
  xValue = (d) -> d.date
  yValue = (d) -> parseFloat(d.avg_diff)
  ...
  chart = (selection) ->
    selection.each (rawData) ->
      ...
      svg = d3.select(this).selectAll("svg").data(filteredData)
      gEnter = svg.enter().append("svg").attr("id", (d) -> d.city + "_diff").append("g")
  ...
  lineGraph = (city, data) ->
    ...
    t = d3.select("##{city}_diff").selectAll(".request")
      .transition()
      .duration(duration)
  ...
  update = () ->
    ...
    svg.each((d) -> lineGraph(d.city, d.colors))
